...
class ChatController extends Controller
{
    ...
    {
        $user = $this->getUser();
        $find = $this->getDoctrine()->getRepository('AppBundle:Chat')->findOneBy(array('chatPartie' => $id),array('id' => 'desc'));
//        dump($find);
        $response = new Response();
        $response->setContent(json_encode(array(
//            'id' => $find->getId(),
            'message' => $find->getChatMessage(),
            'joueur' => $find->getChatJoueur()->getUsername(),
            'joueurId' => $find->getChatJoueur()->getId(),
        )));
...
//        $message = $request->request->get('text');
        ...
        $chat = new Chat();
        $chat->setChatPartie($partie);
        $chat->setChatJoueur($user);
